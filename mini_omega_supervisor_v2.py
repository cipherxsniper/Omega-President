...
# ==============================
# Optional psutil for optimization (skip on Android)
# ==============================
try:
    import psutil
except ImportError:
    psutil = None
    print("[WARNING] psutil not found — CPU/memory optimization disabled")
...
def repair_node(node_id):
    ...
    try:
        ...
        if result.returncode != 0:
            logging.warning(f"Syntax error in Node {node_id}, appending newline as quick fix...")
...
def start_nodes_rotating(node_processes):
...
def supervisor_loop(node_processes):
    while True:
...
def main():
    print("=== Mini-Omega Supervisor v2 ===")
    ...
    node_processes = {}
    start_nodes_rotating(node_processes)

    threading.Thread(target=supervisor_loop, args=(node_processes,), daemon=True).start()
    ...
    while True:
        ...
        if cmd == "help":
            ...
            print("Shutting down Mini-Omega...")
